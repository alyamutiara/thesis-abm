...
globals [
  speed         ; walking speed of the agent
]
...
people-own [
  curr-exit     ; exit door currently choosen
  curr-dir      ; current direction of a agent
...
to people-move
  ;; people random movements
  ask people [
    let target-heading 0

    ;; agents keep moving until there's another agent ahead within speed radius
    if not any? other turtles-on patch-ahead speed [
      forward speed
      set state "walk"
    ]
